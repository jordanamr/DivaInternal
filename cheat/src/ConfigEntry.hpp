...
#define CONFIG_ENTRY(Type, Name, DefaultValue) ConfigEntry<Type> Name = ConfigEntry<Type>(#Name, DefaultValue, false);
#define CONFIG_ENTRY_HOTKEY(Type, Name, DefaultValue, DefaultHotkey) ConfigEntry<Type> Name = ConfigEntry<Type>(#Name, DefaultValue, true, DefaultHotkey);

template<typename T>
class ConfigEntry
{
public:
	ConfigEntry(const std::string& name, const T& defaultValue, bool hasHotKey = false, const Hotkey& defaultHotkey = Hotkey())

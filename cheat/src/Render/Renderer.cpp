...
#pragma region DX11
	...
	void OnRenderDX11(ID3D11DeviceContext* pContext)
	{
		...
		// Get screen position for watermark
		ImDrawList* drawList = ImGui::GetBackgroundDrawList();
		ImVec2 pos = ImVec2(10, 10); // Top-left corner

		const char* watermarkText = xorstr("DivaInternal Injected - Menu key: F6");
		ImVec2 textSize = ImGui::CalcTextSize(watermarkText);
		ImVec2 padding = ImVec2(5, 3); // Padding around text

		// Draw black background rectangle
		drawList->AddRectFilled(
			ImVec2(pos.x - padding.x, pos.y - padding.y), // Top-left
			ImVec2(pos.x + textSize.x + padding.x, pos.y + textSize.y + padding.y), // Bottom-right
			IM_COL32(0, 0, 0, 255), // Solid black
			5.0f // Rounded corners
		);

		// Draw text
		drawList->AddText(pos, IM_COL32(255, 255, 255, 255), watermarkText);

		if (gui.IsOpen())
			gui.Render();

		// TODO: Key Rebind
		if (Utils::KeyPressed(VK_F6))
			gui.Toggle();
